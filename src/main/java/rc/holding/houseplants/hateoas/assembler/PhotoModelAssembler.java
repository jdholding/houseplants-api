...
public class PhotoModelAssembler extends RepresentationModelAssemblerSupport<Photo, PhotoModel> {
  ...
  public PhotoModelAssembler() {
    super(PhotoController.class, PhotoModel.class);
  }

  @Override
  protected PhotoModel instantiateModel(Photo photo) {
  ...
  public PhotoModel toModel(Photo photo) {
    ...
    model.add(linkTo(methodOn(PhotoController.class).getImage(photo.getId())).withRel("image"));
    model.add(
        linkTo(methodOn(PhotoController.class).getThumbnail(photo.getId())).withRel("thumbnail"));
